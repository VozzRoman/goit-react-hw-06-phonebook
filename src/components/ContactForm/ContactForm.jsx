  ...
  return (
    ...
    <Forms onSubmit={handleSubmit(onFormSubmit)}>
      <FildName htmlFor="name">
        ...
        <EnterName
		  {...register('name', {
			...
			pattern: {
				...
				message: 'Name may contain only letters, apostrophe, dash and spaces. For example Adrian, Jacob Mercer, Charles de Batz de Castelmore dArtagnan'
		  }})}
          id="name"
          type="text"
          name="name"
		...
		{errors?.name && <span style={{display: "block", marginBottom: 5, color: 'red'}}>{errors?.name?.message || "Error"}</span>}
      <FildName htmlFor="number">
        ...
        <EnterName
		  {...register('number', {
			...
			pattern: {
				value: /\d{3}[-]\d{2}[-]\d{2}/,
				message: "Phone number must be digits and can contain spaces, dashes, parentheses"
			}
		})}
          id="number"
          type="tel"
